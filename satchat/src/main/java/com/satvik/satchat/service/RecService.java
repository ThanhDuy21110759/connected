...
public class RecService {
  ...
  public PageResponse<PostResponse> getRecPosts(int page, int size)
    ...
    String redisKey = "spring-app::rec-posts::" + userEntity.getId();
    ...
    if (cachedPosts != null) {
      ...
      redisTemplate.opsForValue().set(redisKey, jsonList, Duration.ofMinutes(5));
    }
  ...
  public PageResponse<ProductResponse> getRecProducts(int page, int size)
    ...
    UserEntity userEntity =
        userRepository
            .findByUsername(userDetails.getUsername())
            .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
    String redisKey = "spring-app::rec-products::" + userEntity.getId();
